...
class CommandsTestCases(unittest.TestCase):
    ...
    def test_game_commands_output(self, mock_print):
        ...
        self.assertEqual(self.commands._game.get_player_from_level().get_health(), 9)
        self.commands.do_game_info(None)
    ...
    def test_game_commands_input(self, mock_input):
        ...
        self.commands.do_next_turn(None)
        self.assertNotEqual({self.commands._game.get_player_from_level().get_x(),
                             self.commands._game.get_player_from_level().get_y()}, {0, 0})
    ...
    def test_when_do_next_turn_player_health_zero(self, mock_input, mock_print):
        self.commands = Commands()
        self.commands._game.get_player_from_level().set_health(-1)
        mock_input.side_effect = ['1', '1', 'No', 'attack']
        self.commands.do_next_turn(None)
    ...
    def test_when_do_load_game_options_command_exception(self, mock_print):
        self.commands = Commands()
        self.commands.do_load_game_options("hello")
    ...
    def test_when_do_load_game_options_command(self):
        self.commands = Commands()
        self.commands.do_load_game_options("testing")
        expected = 9
        actual = self.commands._game.get_player_from_level().get_health()
        self.assertEqual(expected, actual)
    ...
    def test_when_do_save_game_command(self, mock_input):
        ...
        self.commands.do_save_game("testing-game")
        expected = False
    ...
    def test_when_do_load_game_command(self):
        self.commands = Commands()
        self.commands.do_load_game("testing-game")
        self.assertNotEqual({self.commands._game.get_player_from_level().get_x(),
                             self.commands._game.get_player_from_level().get_y()}, {0, 0})

    def test_when_do_EOF_command(self):
        self.commands = Commands()
        self.assertTrue(self.commands.do_EOF(None))

    @patch('builtins.print')
    def test_when_preloop_command(self, mock_print):
        self.commands = Commands()
        self.commands.preloop()
    ...
    def test_when_instantiate_commands_constructor(self):
        ...
        commands = Commands()
        self.assertEqual(6, commands._game.get_player_from_level().get_health())
